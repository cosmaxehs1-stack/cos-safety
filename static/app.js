...
async function loadManagePage() {
    ...
    list.innerHTML = '<span style="color:#888;">불러오는 중...</span>';
    try {
        const res = await fetch("/api/channels/status", { headers: authHeaders() });
        if (res.status === 401) { logout(); return; }
        const data = await res.json();
        let html = '<table class="data-table"><thead><tr><th>채널</th><th>건수</th><th style="width:80px;">삭제</th></tr></thead><tbody>';
        data.channels.forEach(ch => {
            const count = data.counts[ch] || 0;
            html += '<tr><td>' + escapeHtml(ch) + '</td><td>' + (count > 0 ? count + '건' : '<span style="color:#aaa;">미업로드</span>') + '</td>'
                + '<td><button class="btn-del" ' + (count === 0 ? 'disabled' : '') + ' onclick="deleteChannelFromManage(\'' + escapeHtml(ch).replace(/'/g, "\\'") + '\')">삭제</button></td></tr>';
        });
        html += '</tbody></table>';
        list.innerHTML = html;
        document.getElementById("manage-channel-total").textContent = "전체 " + data.total + "건";

        // 채널 검색 필터 옵션
        const sel = document.getElementById("manage-search-channel");
        sel.innerHTML = '<option value="">전체 채널</option>';
        data.channels.forEach(ch => {
            sel.innerHTML += '<option value="' + escapeHtml(ch) + '">' + escapeHtml(ch) + '</option>';
        });
    } catch (e) {
        list.innerHTML = '<span style="color:#e74c3c;">불러오기 실패</span>';
    }

    // 레코드 테이블 초기화
    document.getElementById("manage-records-tbody").innerHTML = '';
    document.getElementById("manage-records-info").textContent = '검색어를 입력하거나 검색 버튼을 눌러주세요.';
}

async function deleteChannelFromManage(channel) {
    if (!confirm("[" + channel + "] 데이터를 전부 삭제하시겠습니까?")) return;
    try {
        const res = await fetch("/api/channels/delete", {
            method: "POST",
            headers: { ...authHeaders(), "Content-Type": "application/json" },
...
async function searchManageRecords() {
    const keyword = document.getElementById("manage-search-input").value.trim();
    const channel = document.getElementById("manage-search-channel").value;
    const tbody = document.getElementById("manage-records-tbody");
    const info = document.getElementById("manage-records-info");
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:20px;color:#888;">검색 중...</td></tr>';

    try {
        let url = "/api/summary?page=records";
        if (channel) url += "&channel=" + encodeURIComponent(channel);
        if (keyword) url += "&keyword=" + encodeURIComponent(keyword);
        const res = await fetch(url, { headers: authHeaders() });
        if (res.status === 401) { logout(); return; }
        const data = await res.json();
        _manageRecords = data.records || [];

        if (_manageRecords.length === 0) {
            tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:20px;color:#888;">검색 결과가 없습니다.</td></tr>';
            info.textContent = '0건';
            return;
        }

        let html = '';
        _manageRecords.forEach(r => {
            html += '<tr>'
                + '<td><input type="checkbox" class="manage-check" value="' + (r._id || '') + '"></td>'
                + '<td>' + (r.no || '') + '</td>'
                + '<td>' + escapeHtml(r.channel || '') + '</td>'
                + '<td>' + escapeHtml(r.date || '') + '</td>'
                + '<td>' + escapeHtml(r.location || '') + '</td>'
                + '<td>' + escapeHtml(r.content || r.content_full || '').substring(0, 40) + '</td>'
                + '<td>' + escapeHtml(r.person || '') + '</td>'
                + '<td>' + escapeHtml(r.completion || '') + '</td>'
                + '<td><button class="btn-del" onclick="deleteSingleRecord(\'' + (r._id || '') + '\')">삭제</button></td>'
                + '</tr>';
        });
        tbody.innerHTML = html;
        info.textContent = _manageRecords.length + '건';
        document.getElementById("manage-check-all").checked = false;
    } catch (e) {
        tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:20px;color:#e74c3c;">검색 실패</td></tr>';
    }
}

function toggleManageCheckAll(el) {
    document.querySelectorAll(".manage-check").forEach(cb => { cb.checked = el.checked; });
}
...
async function deleteSelectedRecords() {
    ...
    if (checked.length === 0) { alert("삭제할 레코드를 선택해주세요."); return; }
    ...
    for (const cb of checked) {
        try {
            const res = await fetch("/api/record/delete", {
                method: "POST",
                headers: { ...authHeaders(), "Content-Type": "application/json" },
                body: JSON.stringify({ _id: cb.value }),
            });
            if (res.ok) success++; else fail++;
        } catch (e) { fail++; }
    }
    alert("삭제 완료: " + success + "건" + (fail > 0 ? ", 실패: " + fail + "건" : ""));
    searchManageRecords();
    loadManagePage();
    fetchSummary();
}
